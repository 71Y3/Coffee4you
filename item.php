...
$result1=mysql_query("SELECT * FROM table_product $qury_start_num");
echo mysql_error();
if (mysql_num_rows($result1) > 0)
{
...
<button class="btn_add_of_basket" tid="'.$row["products_id"].' method="post">В корзину</button>
</div>
</div>';
...
if ($page != 1){ $pstr_prev = '<li><a class="pstr-prev" href="item.php?page='.($page - 1).'">&lt;</a></li>';}
if ($page != $total) $pstr_next = '<li><a class="pstr-next" href="item.php?page='.($page + 1).'">&gt;</a></li>';

// Формируем ссылки со страницами
if($page - 5 > 0) $page5left = '<li><a href=item.php?page='.($page - 5).'">'.($page - 5).'</a></li>';
if($page - 4 > 0) $page4left = '<li><a href=item.php?page='.($page - 4).'">'.($page - 4).'</a></li>';
if($page - 3 > 0) $page3left = '<li><a href=item.php?page='.($page - 3).'">'.($page - 3).'</a></li>';
if($page - 2 > 0) $page2left = '<li><a href=item.php?page='.($page - 2).'">'.($page - 2).'</a></li>';
if($page - 1 > 0) $page1left = '<li><a href=item.php?page='.($page - 1).'">'.($page - 1).'</a></li>';

if($page + 5 <= $total) $page5right = '<li><a href="item.php?page='.($page + 5).'">'.($page + 5).'</a></li>';
if($page + 4 <= $total) $page4right = '<li><a href="item.php?page='.($page + 4).'">'.($page + 4).'</a></li>';
if($page + 3 <= $total) $page3right = '<li><a href="item.php?page='.($page + 3).'">'.($page + 3).'</a></li>';
if($page + 2 <= $total) $page2right = '<li><a href="item.php?page='.($page + 2).'">'.($page + 2).'</a></li>';
if($page + 1 <= $total) $page1right = '<li><a href="item.php?page='.($page + 1).'">'.($page + 1).'</a></li>';


if ($page+5 < $total)
{
    $strtotal = '<li><p class="nav-point">...</p></li><li><a href="item.php?page='.$total.'">'.$total.'</a></li>';
}else
{
    $strtotal = ""; 
}
