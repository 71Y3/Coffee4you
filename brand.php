...
switch ($orderby)
{
    ...
    default:
    $orderby = 'count';
    $orderby_name = 'Нечего не выбрано';
    break;
}
?>
        ...
        <div class="widget-area basel-sidebar-content">
            <div id="basel-woocommerce-sort-by-4" class="sidebar-widget basel-woocommerce-sort-by">
                <h3 class="widget-title">Сортировать</h3>
                        ...
                        <li><a href="brand.php?cat='.$cat.'&type='.$type.'&orderby=popularity" >Популярные</a></li>
                        <li><a href="brand.php?cat='.$cat.'&type='.$type.'&orderby=date" >Новинки</a></li>
                        <li><a href="brand.php?cat='.$cat.'&type='.$type.'&orderby=price-asc" >Цена: сначала дешевые</a></li>
                        <li><a href="brand.php?cat='.$cat.'&type='.$type.'&orderby=price-desc" >Цена: сначала дорогие</a></li></ul>
                        </form> 
                        </div>
                        </div>';
                        ?>
...
{
    $querycat = "AND brand='$cat' AND category='$type'";
}else
...
if ($page != 1){ $pstr_prev = '<li><a class="pstr-prev" href="brand.php?cat='.$cat.'&type='.$type.'&page='.($page - 1).'">&lt;</a></li>';}
if ($page != $total) $pstr_next = '<li><a class="pstr-next" href="brand.php?cat='.$cat.'&type='.$type.'&page='.($page + 1).'">&gt;</a></li>';

// Формируем ссылки со страницами
if($page - 5 > 0) $page5left = '<li><a href=brand.php?cat='.$cat.'&type='.$type.'&page='.($page - 5).'">'.($page - 5).'</a></li>';
if($page - 4 > 0) $page4left = '<li><a href=brand.php?cat='.$cat.'&type='.$type.'&page='.($page - 4).'">'.($page - 4).'</a></li>';
if($page - 3 > 0) $page3left = '<li><a href=brand.php?cat='.$cat.'&type='.$type.'&page='.($page - 3).'">'.($page - 3).'</a></li>';
if($page - 2 > 0) $page2left = '<li><a href=brand.php?cat='.$cat.'&type='.$type.'&page='.($page - 2).'">'.($page - 2).'</a></li>';
if($page - 1 > 0) $page1left = '<li><a href=brand.php?cat='.$cat.'&type='.$type.'&page='.($page - 1).'">'.($page - 1).'</a></li>';

if($page + 5 <= $total) $page5right = '<li><a href="brand.php?cat='.$cat.'&type='.$type.'&page='.($page + 5).'">'.($page + 5).'</a></li>';
if($page + 4 <= $total) $page4right = '<li><a href="brand.php?cat='.$cat.'&type='.$type.'&page='.($page + 4).'">'.($page + 4).'</a></li>';
if($page + 3 <= $total) $page3right = '<li><a href="brand.php?cat='.$cat.'&type='.$type.'&page='.($page + 3).'">'.($page + 3).'</a></li>';
if($page + 2 <= $total) $page2right = '<li><a href="brand.php?cat='.$cat.'&type='.$type.'&page='.($page + 2).'">'.($page + 2).'</a></li>';
if($page + 1 <= $total) $page1right = '<li><a href="brand.php?cat='.$cat.'&type='.$type.'&page='.($page + 1).'">'.($page + 1).'</a></li>';


if ($page+5 < $total)
{
    $strtotal = '<li><p class="nav-point">...</p></li><li><a href="brand.php?cat='.$cat.'&type='.$type.'&page='.$total.'">'.$total.'</a></li>';
}else
{
    $strtotal = ""; 
}
...
if ($total > 1)
{
    ...
    <ul>
    ';
    echo $pstr_prev.$page5left.$page4left.$page3left.$page2left.$page1left."<li><a class='pstr-active' href='brand.php?cat='.$cat.'&type='.$type.'&page=".$page."'>".$page."</a></li>".$page1right.$page2right.$page3right.$page4right.$page5right.$strtotal.$pstr_next;
    echo '
    </ul>
    </div>
    </div>
